...
import time
from utils import quaternion_from_euler, lonlat2xyz, quat2euler
...
class ExtendedKalmanFilter(Node):
    ...
    def fvq(self, q1, q2, q3, q4, ax, ay, az):
        return np.array([
    ...
    def hxq(self,rgps,q1,q2,q3,q4):
        return np.array([
            [(-rgps*2*q1),(-rgps*2*q2),(rgps*2*q3),(rgps*2*q4)],
            [(-rgps*2*q4),(-rgps*2*q3),(-rgps*2*q2),(-rgps*2*q1)],
            [(rgps*2*q3),(-rgps*2*q4),(rgps*2*q1),(-rgps*2*q2)]
        ])
    
    def hvq(self,rgps,q1,q2,q3,q4,Q,R):
        return np.array([
        [(2*rgps*q3*Q + 2*rgps*q4*R), (2*rgps*q4*Q - 2*rgps*q3*R), (2*rgps*q1*Q - 2*rgps*q2*R), (2*rgps * q2 * Q + 2 * rgps * q1 * R)],
        [(-2*rgps*q2*Q - 2*rgps*q1*R), (2*rgps*q2*R - 2*rgps*q1*Q), (2*rgps*q4*Q - 2*rgps*q3*R), (2*rgps * q3 * Q + 2 * rgps * q4 * R)],
        [(2*rgps*q1*Q - 2*rgps*q2*R), (-2*rgps*q2*Q - 2*rgps*q1*R),(-2*rgps*q3*Q - 2*rgps*q4*R), (2*rgps * q4 * Q - 2 * rgps * q3 * R)]
        ])
    
    def A_mat(self,Fvq,Fqq,Fqb,Fvb):
        row1 = np.hstack([self.Z, self.I, self.Z34, self.Z, self.Z])
        row2 = np.hstack([self.Z, self.Z, Fvq, self.Z, Fvb])
        row3 = np.hstack([self.Z43, self.Z43, Fqq, Fqb, self.Z43])
        row4 = np.hstack([self.Z, self.Z, self.Z34, self.Z, self.Z])
        row5 = np.hstack([self.Z, self.Z, self.Z34, self.Z, self.Z])
        # print(row1.shape,row2.shape,row3.shape,row4.shape,row5.shape)
        return np.vstack([row1, row2, row3, row4, row5])
    
    def H_mat(self,Hxq,Hvq):
        row1 = np.hstack([self.I, self.Z, Hxq, self.Z36])
        row2 = np.hstack([self.Z, self.I, Hvq, self.Z36])
    ...
    def ekf_function(self):
        ...
        omg_w = np.array([  [0      ,self.p ,self.q ,self.r],
        ...
        self.quat = np.array([[self.xhat[6,0], self.xhat[7,0], self.xhat[8,0], self.xhat[9,0]]]).T
        # .. Normailize quat
        magnitude = np.linalg.norm(self.quat)
        ...
        #delV/delQ
        # print(self.quat[0],self.quat[1],self.quat[2],self.quat[3])
        Fvq = self.fvq(self.quat[0,0],self.quat[1,0],self.quat[2,0],self.quat[3,0],self.measure[3],self.measure[4],self.measure[5]) # ..
        ...
        #delQ/del_gyrobias
        Fqb = (0.5)*self.fqb(self.quat[0,0],self.quat[1,0],self.quat[2,0],self.quat[3,0]) # ..
        
        # Now assemble the Transition matrix A

        A = self.A_mat(Fvq,Fqq,Fqb,Fvb)# ..
        ...
        # del v/del q
        # print(self.rgps[0])
        Hvq = self.hvq(self.rgps[0],self.quat[0,0],self.quat[1,0],self.quat[2,0],self.quat[3,0],self.q, self.r) # ..
        
        #del P/del q
        Hxq = self.hxq(self.rgps[0],self.quat[0,0],self.quat[1,0],self.quat[2,0],self.quat[3,0]) # ..
        # Assemble H
        
        H = self.H_mat(Hxq,Hvq) # ..
