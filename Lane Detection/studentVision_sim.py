...
class lanenet_detector():
    ...
    def combinedBinaryImage(self, img):
        ...
        binaryImage = cv2.add(SobelOutput,ColorOutput)
    ...
    def perspective_transform(self, img, verbose=False):
        ...
        # dst = np.float32([[75, 0], [img.shape[1], 0], [75, img.shape[0]], [img.shape[1], img.shape[0]]])
        # img= img[0 : 400, 0 : 640]

        src = np.float32([[260, 270], [385, 270], [0, 395], [640, 395]])
        dst = np.float32([[0, 0], [img.shape[1], 0], [0, img.shape[0]], [img.shape[1], img.shape[0]]])
        # sim
        # src = np.float32([[90, 350], [570, 350], [0, 400], [620, 400]])
        #src = np.float32([[230, 285], [400, 275], [50, 400], [600, 430]])
        # src = np.float32([[235, 285], [400, 280], [10, 420], [640, 420 ]])

        # src = np.float32([[250, 280], [400, 280], [0, 400], [620, 400]])
        #2. Get M, the transform matrix, and Minv, the inverse using cv2.getPerspectiveTransform()
        # offset = 75
        # dst = np.float32([[offset, 0], [img.shape[1]-offset, offset], [offset, img.shape[0]+offset], [img.shape[1]-offset, img.shape[0]+offset]])

        # dst = np.float32([[0, 0], [img.shape[1]/2, 0], [0, img.shape[0]/2], [img.shape[1]/2, img.shape[0]/2]])
        # dst = np.float32([[0, 0], [img.shape[1], 0], [0, img.shape[0]], [img.shape[1], img.shape[0]]])
        ...
        warped_img = cv2.copyMakeBorder(warped_img, 0, 0, 100, 100, cv2.BORDER_CONSTANT, None, value = 0)
        warped_img = cv2.resize(warped_img, (640, 480))
        
        cv2.imwrite("bird_from_cv2.png",warped_img)
